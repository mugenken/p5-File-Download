...
sub download {
    ...
    my $res = $ua->request(
        ...
        sub {
            ...
            unless ( defined $self->file ) {
                ...
                unless ( defined $self->outfile ) {
                    ...
                    unless ($self->file) {
                        ...
                        $self->file( ( $rurl->path_segments )[-1] );

                        if ( !defined($self->file) || !length($self->file) ) {
                            $self->file( "index" );
                            my $suffix = media_suffix( $res->content_type );
                            $self->file( $self->file . ".$suffix") if $suffix;
                        }
                        elsif ($rurl->scheme eq 'ftp'
                            || $self->file =~ /\.t[bg]z$/
                            || $self->file =~ /\.tar(\.(Z|gz|bz2?))?$/ )
                        {

                            # leave the filename as it was
                        }
                        else {
                            my $ct = guess_media_type($self->file);
               ...
               # might try to trick us into doing something bad.
                    if ( $self->file
                        && !length($self->file)
                        || $self->file =~ s/([^a-zA-Z0-9_\.\-\+\~])/sprintf "\\x%02x", ord($1)/ge)
                    {
                        die "Will not save <$url> as \"" . $self->file . "\".\nPlease override file name on the command line.\n";
                    }

                    if ( defined $directory ) {
                        require File::Spec;
                        $self->file = File::Spec->catfile( $directory, $self->file );
                    }

                    # Check if the file is already present
                    if ( -l $self->file ) {
                        die "Will not save <$url> to link \"" . $self->file . "\".\nPlease override file name on the command line.\n";
                    }
                    elsif ( -f _ ) {
                        die "Will not save <$url> as \"" . $self->file . "\" without verification.\nEither run from terminal or override file name on the command line.\n"
                          unless -t;
                        return 1 if ( !$self->{overwrite} );
                    }
                    elsif ( -e _ ) {
                        die
                          "Will not save <$url> as \"" . $self->file . "\".  Path exists.\n";
                    }
                    else {
                        $self->status("Saving to '" . $self->file . "'...\n");
                    }
                }
                else {
                    $self->file( $self->file );
                ...
                open( my $fh, '>', $self->file ) || die "Can't open $self->file: $!\n";
                binmode $fh unless $self->mode eq 'a';
            ...
            print FILE $_[0] or die "Can't write to $self->file: $!\n";
            $self->size( $self->size + length( $_[0] ) );
            ...
            if ( defined $self->length ) {
                ...
                if ( $dur != $self->last_dur ) {    # don't update too often
                    ...
                    my $speed;
                    $speed = fbytes( $self->size / $dur ) . "/sec"
                      if $dur > 3;
                    my $secs_left = fduration( $dur / $perc - $dur );
                    $perc = int( $perc * 100 );
                    $self->status( "$perc% of " . $self->flength );
                    $self->status( $self->status . " (at $speed, $secs_left remaining)" ) if $speed;
                }
            }
            else {
    ...
    if ( fileno(FILE) ) {
        close(FILE) || die "Can't write to $self->file: $!\n";
        ...
        if ( $res->header("X-Died") || !$res->is_success ) {
            ...
            if (-t) {
                if ( $self->{autodelete} ) {
                    ...
                    $self->status(
                        "Aborted. Truncated file kept: "
                        . fbytes( $self->length - $self->size )
                        . " missing"
                    );
                }
                return 1;
                ...
                $self->status( "Transfer aborted, $self->file kept" );
            }
        }
        return 0;
    }
